...
export default function ChatSidebar({ user, chats }) {
  ...
  const filteredChats = useMemo(() => {
    ...
    return chats.filter(
      ...
      // some is a built-in array method used to check if at least one element in an array passes a specific condition
    );
  }, [chats, searchQuery]);

  // Groups chats by date (today, yesterday, month, year)
    ...
    return chatList.map((chat) => (
      <Fragment key={chat.id}>
        <Link
          href={`/chat/${chat.id}`}
          className={cn(
            "block rounded-1g px-3 py-2 text-sm text-sidebar-foreground hover:bg-sidebar-accent transition-colors",
